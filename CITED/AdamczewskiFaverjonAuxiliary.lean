...
which costs a constant `2^{m²⌈log₂(2m²)⌉}` depending on `m` alone — and only constants independent
of `δ₁` matter for the final contradiction.  This is `AF.relDim_le_of_le_two_pow`.
